...
import love.forte.simbot.component.onebot.v11.core.api.OneBotApiResult.Companion.SUCCESS_RETCODE
...
public interface OneBotApi<T : Any> {
     ...
     */
    public val apiResultDeserializer: DeserializationStrategy<T>
 ...
 * 同时在反序列化时，如果 `retcode` == [SUCCESS_RETCODE] 并且结果的预期类型是一个 [Unit]，则直接填充它。
 * 建议
 *
...
public data class OneBotApiResult<T : Any>(
    ...
    public val isSuccess: Boolean
        get() = retcode == SUCCESS_RETCODE

    public val dataOrThrow: T
        get() = data
            ?: when (retcode) {
                ASYNC_RETCODE -> throw IllegalStateException(
                    "`data` is null, " +
    ...
    public companion object {
         ...
         */
        public const val ASYNC_RETCODE: Int = 1
...
private object EmptyOneBotApiResultSerializer : KSerializer<OneBotApiResult<Unit>> {
    private val DEFAULT_EMPTY_SUCCESS_RESULT = OneBotApiResult(retcode = SUCCESS_RETCODE, status = "ok", data = Unit)
            ...
            if (decodeSequentially()) {
                ...
                if (retcode == DEFAULT_EMPTY_SUCCESS_RESULT.retcode && status == DEFAULT_EMPTY_SUCCESS_RESULT.status) {
                    ...
                    OneBotApiResult(
                        retcode = retcode,
                        status = status,
                        data = if (retcode == SUCCESS_RETCODE) Unit else null
                    )
                }
